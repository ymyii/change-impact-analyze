...
code_refs:
  ...
  - path: "analyzer/src/main/java/io/github/dependencyanalysis/impact/ArtifactPathBindingResolver.java"
    desc: "baseline/target Module-local coordinate-only path lookup"
...
`impact` 同时需要 Maven mediated dependency tree 与 exact physical artifact path。每个 workspace 在原 project 的同一个 Maven process/session 中先执行 Maven Dependency Plugin `tree`，再执行内置 Artifact Path Plugin。GraphML 是唯一 mediation authority，决定 selected coordinate 与 effective scope；Module-local Schema v2 JSON 只保存 selected non-Reactor dependency 的 coordinates 与 physical absolute path，不包含 Module 或 scope。Production path 不生成 temporary POM、不调用 `dependency:list`、不拼接 local repository path，也不添加 `-llr`。
...
- Duplicate coordinate 不因 scope 不同而失败；同一 coordinates 只有在最终 canonical paths 不同时才报告 physical path ambiguity。
