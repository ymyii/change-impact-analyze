...
`impact` 同时需要 Maven mediated dependency tree 与 exact physical artifact path。GraphML 保留 dependency graph；pinned Maven Dependency Plugin `3.6.1:list` 提供 absolute artifact filename。生产路径不再从 `~/.m2` 拼接 JAR 路径。

## Commands

- Tree：`mvn ... dependency:tree -DoutputType=graphml -DoutputFile=<unique>.graphml -B`
