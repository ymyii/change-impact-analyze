...
code_refs:
  ...
  - path: "analyzer/src/main/java/io/github/dependencyanalysis/diagnostic/DiagnosticContext.java"
    desc: "并发任务的stage、substage与stable identity上下文"
...
- 指标监控必须定义能够解释主要执行成本的工作量、完成量、阶段耗时、并发或队列状态、资源使用量及异常计数；指标必须绑定明确的任务和阶段，不能只提供缺少上下文的全局数值。
- 进度跟踪必须覆盖开始、处理中、完成和失败状态，并提供稳定任务标识、阶段、完成量、总量、耗时与结果。无法预知总量时必须输出已完成量和当前阶段，不能伪造百分比。
- 审计日志必须记录关键状态转换、影响执行路径或成本的决策、执行结果和异常，使同一次任务的阶段顺序、状态变化及耗时能够被还原。
...
- `WARN` 和 `ERROR` 保留足以识别任务、阶段、状态与原因的低成本摘要；高成本诊断详情必须拆分到已启用的 `DEBUG` 或 `TRACE`。
...
- 不同级别的事件必须共享可关联的任务标识、`stage`、`substage`、时间与状态语义；耗时使用 monotonic timing，时间戳用于事件排序与跨输出关联。
- 并发任务必须使用稳定业务 identity，不能依赖 thread name 区分；进度、指标、审计事件和异常必须能够精确归属到同一任务。
- message 承载工作量、耗时、状态、结果和指标值；日志 prefix 只承载稳定身份字段，遵守既有 `DiagnosticContext` 与 `DiagnosticLog` 格式契约。
...
- CLI command、分析 pipeline、并发或后台任务、外部 process、缓存处理和Report生成等存在阶段性执行成本的路径。
...
- 检查每项输出都具有任务和阶段关联信息，能够从开始、处理中、完成或失败事件还原一次执行，并计算主要阶段耗时。
...
- `DiagnosticContext` 提供并发任务稳定的阶段与 identity 上下文。
