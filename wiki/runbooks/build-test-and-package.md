...
6. 执行 Analyzer Maven quality gate。

   ```sh
   mvn clean verify
   ```

7. 首次执行或 lockfile 变化后安装 Node.js dependency 与 Chromium。
...
8. 在 Maven fixture 已生成后执行 Offline Report browser gate。

   ```sh
   npm run test:report
   ```

9. 验证全部 public command help。
...
- Plugin repository ZIP resolution failure：重新执行 `mvn -f plugins/pom.xml clean install`，确认 root `artifact-path-plugin.version` 一致。
...
- Playwright fixture missing：先完成 `mvn clean verify`；浏览器缺失时执行 `npx playwright install chromium`。
