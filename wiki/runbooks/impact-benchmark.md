...
- 每个 scope 完成 1 次 warm-up 与 5 次 formal，双 scope 共 12 个 JVM process。
- 同一 scope 的 topology gate 通过，两个 semantic baseline 均非 `PENDING`。
- 两份 HTML 通过 verification，publisher 一次性提交 `changed-paths` 与 `full` snapshot。

## Failure Entrypoints

- 未获得用户当前明确授权：停止，不执行任何 benchmark runner。
- Calibration baseline 仍为 `PENDING`：保留 candidate，仅在人工确认后更新 expected baseline。
- 任一 process、topology、semantic、HTML 或 publication gate 失败：不发布任何 tracked snapshot，并检查 `tmp-files/impact-medium-benchmark/` 中对应 run artifact。
