...
| `BENCHMARK_RESULT_REFINEMENT_ALGORITHMS` | 无 | suite 按 SSA 或 CHA local control 设置 |
...
当前不保留历史 tracked TSV；只有两个 scope 的 14 个进程全部通过且 topology 稳定时，publisher 才原子写入新快照。`BENCHMARK_CALIBRATION=1` 只生成 candidate 与 HTML。
...
- `expected-results.tsv` 存在当前 scope/refinement baseline；`PENDING` 只允许显式 calibration。
- Affected Call Chains 保留递归、Object dispatch、structural reference、external ancestor 和 dependency path evidence。
- CHA 裁剪无关 external target，不为被裁剪调用生成 dependency boundary limitation。
- local receiver control 保留 `ChangedReceiver`，删除 `unrelatedReceiverPath`，且 pruned edge count 大于零。
