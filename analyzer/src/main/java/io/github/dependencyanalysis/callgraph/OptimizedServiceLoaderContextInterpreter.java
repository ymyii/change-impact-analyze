...
/** Optimized 0-1-CFA-owned ServiceLoader execution and IR cache. */
final class OptimizedServiceLoaderContextInterpreter
        implements SSAContextInterpreter {

    /** Pure summary IR creation. */
    private final OptimizedServiceLoaderSummaryIrFactory factory;

    /** Optimized-local generated IR cache. */
    private final Map<CGNode, IR> irs = new HashMap<>();

    OptimizedServiceLoaderContextInterpreter(
            final OptimizedServiceLoaderModel model) {
        factory = new OptimizedServiceLoaderSummaryIrFactory(model);
    }
