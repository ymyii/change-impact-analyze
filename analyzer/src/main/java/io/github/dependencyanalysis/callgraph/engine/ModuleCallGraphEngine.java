...
import io.github.dependencyanalysis.callgraph.entrypoint.DeclaredTypesEntrypoint;
...
import io.github.dependencyanalysis.callgraph.strategy.CallGraphStrategyFactory;
...
import io.github.dependencyanalysis.callgraph.strategy.cha.ChaAncestorRetentionPolicy;
import io.github.dependencyanalysis.callgraph.strategy.cha.ChaCallGraphRequest;
import io.github.dependencyanalysis.callgraph.strategy.cha.ChaDispatchTargetPolicy;
import io.github.dependencyanalysis.callgraph.strategy.kobj.KObjCallGraphRequest;
import io.github.dependencyanalysis.callgraph.topology.CallGraphTopologyAnalyzer;
...
// Wiki: wiki/c4/code/dependency-analyzer-cli-call-graph-engine.md - Per-Module Call Graph contract
...
public final class ModuleCallGraphEngine {
    ...
    public ModuleCallGraphSession build(
        ...
        final EntrypointClassIndex index = new EntrypointClassScanner().scan(
                input.projectClasses(), entrypointSelection);
        return build(input, index, timeoutSeconds);
    }
    ...
    public ModuleCallGraphSession build(
            ...
            final long timeoutSeconds) {
        return build(input, entrypointIndex, timeoutSeconds, false);
    }
    ...
    public ModuleCallGraphSession build(
        ...
        try {
            ...
            final boolean chaChangedPaths = algorithm == CallGraphAlgorithm.CHA
                    && input.dependencyScope().mode()
                    == DependencyScopeMode.CHANGED_PATHS;
            final ChaAncestorRetentionPolicy ancestorRetention =
                    ChaAncestorRetentionPolicy.create(
                            input, ownership, hierarchy, chaChangedPaths);
            final DependencyBodyBoundary dependencyBoundary =
                    new DependencyBodyBoundary(input.dependencyScope(),
                            input.changes(), ownership, hierarchy,
                            ancestorRetention::retains,
            ...
            final CallGraphBuildContext buildContext =
                    new CallGraphBuildContext(scope, hierarchy, entrypoints,
                            cache, serviceLoaderIndex, dynamicModels, monitor);
            final CallGraphStrategyResult strategyResult;
            try {
                final var strategyRequest = algorithm
                        == CallGraphAlgorithm.CHA
                        ? new ChaCallGraphRequest(ownership, chaTargets,
                                ancestorRetention, dependencyBoundary)
                        : new KObjCallGraphRequest(kObjDepth,
                                reflectionOptions, jdkModel,
                                dependencyBoundary);
                strategyResult = new CallGraphStrategyFactory()
                        .create(algorithm).build(
                                buildContext, strategyRequest);
    ...
    private ClassOwnershipIndex ownership(final ModuleCallGraphInput input)
            throws IOException {
        final ClassOwnershipIndex result = new ClassOwnershipIndex();
        result.addDirectory(input.projectClasses(), CodeOrigin.PROJECT);
    ...
    private AnalysisScope scope(
        ...
        result.addToScope(ClassLoaderReference.Application,
                filteredDirectory(input.projectClasses(), ownership));
        addServiceResources(result, input.projectClasses());
        for (Path path : input.reactorDependencyClasses()) {
            result.addToScope(ClassLoaderReference.Application,
