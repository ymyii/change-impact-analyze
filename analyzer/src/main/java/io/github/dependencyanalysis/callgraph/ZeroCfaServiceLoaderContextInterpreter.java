...
/** ZeroCFA-owned ServiceLoader summary execution and IR cache. */
final class ZeroCfaServiceLoaderContextInterpreter
        implements SSAContextInterpreter {

    /** Pure summary IR creation. */
    private final ZeroCfaServiceLoaderSummaryIrFactory factory;

    /** ZeroCFA-local generated IR cache. */
    private final Map<CGNode, IR> irs = new HashMap<>();

    ZeroCfaServiceLoaderContextInterpreter(
            final ZeroCfaServiceLoaderModel model) {
        factory = new ZeroCfaServiceLoaderSummaryIrFactory(model);
    }
