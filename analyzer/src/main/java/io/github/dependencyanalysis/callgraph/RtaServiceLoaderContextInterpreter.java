...
/** RTA-owned ServiceLoader summary execution and IR cache. */
final class RtaServiceLoaderContextInterpreter
        implements SSAContextInterpreter {

    /** Pure summary IR creation. */
    private final RtaServiceLoaderSummaryIrFactory factory;

    /** RTA-local generated IR cache. */
    private final Map<CGNode, IR> irs = new HashMap<>();

    RtaServiceLoaderContextInterpreter(
            final RtaServiceLoaderModel model) {
        factory = new RtaServiceLoaderSummaryIrFactory(model);
    }
