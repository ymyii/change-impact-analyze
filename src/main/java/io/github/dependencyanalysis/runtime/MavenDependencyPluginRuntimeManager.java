...
public final class MavenDependencyPluginRuntimeManager {
    ...
    public MavenDependencyPluginRuntime prepare(
        ...
        if (!override.isEmpty()) {
            if (!supportsCompleteEvidence(override)) {
            ...
            return new MavenDependencyPluginRuntime(
                    override, goal(override), arguments,
                    null, "");
        }
        try {
    ...
    private void extract(
        ...
        try {
            if (!checksum.equals(resourceChecksum())) {
    ...
    private String readChecksum() throws IOException {
        try (InputStream stream = resource(SHA_RESOURCE)) {
    ...
    private String resourceChecksum()
        ...
        try (InputStream input = resource(ZIP_RESOURCE)) {
            final byte[] buffer = new byte[BUFFER_SIZE];
            int count;
            while ((count = input.read(buffer)) >= 0) {
                if (count > 0) {
