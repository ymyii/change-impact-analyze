...
public final class MavenRuntimeManager {
    ...
    /**
     * Returns the default complete config directory.
     *
    ...
    public MavenRuntimeDescriptor prepare(
        ...
        try {
            ...
            throw new MavenRuntimeException(
                    "Unable to prepare Maven runtime",
                    exception);
        }
    }
    ...
    private MavenRuntimeDescriptor userConfigured(
            ...
            final Path javaHome) {
        final Path absolute = executable
                .toAbsolutePath().normalize();
        if (!Files.isRegularFile(absolute)) {
            throw new MavenRuntimeException(
                    "Maven executable is not a file: "
                            + absolute);
        }
        if (!isWindows()
                && !Files.isExecutable(absolute)) {
            throw new MavenRuntimeException(
                    "Maven executable is not executable: "
                            + absolute);
        }
        return new MavenRuntimeDescriptor(
                MavenRuntimeSource.USER_CONFIGURED,
                absolute, null, javaHome,
                configDir, "");
    }
    ...
    private MavenRuntimeDescriptor embedded(
            ...
            final Path javaHome)
            throws IOException {
        final String checksum = readChecksum();
        final Path versionDir = configDir
                .resolve("runtime")
                .resolve("apache-maven")
                .resolve(EMBEDDED_VERSION);
        final Path runtimeLeaf =
                versionDir.resolve(checksum);
        final Path locks = configDir
                .resolve("locks");
        Files.createDirectories(locks);
        final Path lockPath = locks.resolve(
                "apache-maven-" + EMBEDDED_VERSION
                        + "-" + checksum + ".lock");
        final Path normalizedLock = lockPath
                .toAbsolutePath().normalize();
        final Object jvmLock = JVM_LOCKS
                .computeIfAbsent(normalizedLock,
                        ignored -> new Object());
        ...
        return new MavenRuntimeDescriptor(
                ...
                executable, null, javaHome,
                configDir, checksum);
    }

    private void prepareConfigDir(
            final Path configDir)
            throws IOException {
        ...
        try (InputStream stream = resource(
            ...
            final String value = new String(
                    ...
                    java.nio.charset.StandardCharsets
                            .US_ASCII).trim();
            final String first = value
                    .split("\\s+")[0]
                    .toLowerCase(Locale.ROOT);
            if (!first.matches("[0-9a-f]{128}")) {
                throw new IOException(
                        "Invalid embedded SHA-512");
            }
            return first;
        }
    }
    ...
    private boolean matchesEmbeddedDistribution(
            final Path runtimeLeaf)
            throws IOException {
    ...
    private boolean entryMatches(
            ...
            throws IOException {
        final MessageDigest expected = digest();
        ...
        final MessageDigest actual = digest();
    ...
    private void extract(
        ...
        try {
            extractVerifiedZip(staging, checksum);
    ...
    private void extractVerifiedZip(
            final Path destination,
            final String expectedChecksum)
            throws IOException {
        final String actual = resourceChecksum();
        if (!expectedChecksum.equals(actual)) {
            throw new IOException(
                    "Embedded Maven SHA-512 mismatch");
        }
        try (InputStream raw = resource(ZIP_RESOURCE);
             ZipInputStream zip =
    ...
    private void deleteManagedLeaf(
        ...
        final Path normalizedVersion = versionDir
                .toAbsolutePath().normalize();
        if (!normalizedLeaf.getParent()
                .equals(normalizedVersion)) {
            throw new IOException(
                    "Refusing unmanaged runtime cleanup");
        }
