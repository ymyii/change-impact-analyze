...
1. 阅读 `Task Frame`、`Project Profile` 和已确认环境/账号/数据前提，确认完整 Acceptance Criteria。
...
3. 服务未启动时，根据项目文档或自身探索到的运行入口自主启动。
4. 验证截图、日志、响应片段或其他证据写入 `Workflow Temp Dir`。
5. 无法启动、无法访问入口或缺少外部条件时，记录阻塞事实、原因和受影响 Acceptance Criteria。
...
2. 通过时输出关键证据。
3. 不通过或阻塞时输出入口、观察结果、影响、问题和证据。
4. 输出不展开完整操作流水。

## 约束

- **禁止** 接收或使用 `本轮焦点`、设计材料、实现摘要、diff、代码路径、测试 PASS、review 问题或内部审查原因作为验证依据。
- **禁止** 使用 mock 替代真实入口验证。
